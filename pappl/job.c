...
//
// '_papplJobCopy()' - Create a copy of an existing Job object.
//
    ...
    pappl_job_t	    *parent_job)			// I - Existing Job to be copied
{
  ...
  if (NULL == parent_job)
  {
    papplLog(client->system, PAPPL_LOGLEVEL_ERROR, "parent_job is NULL (%s)", strerror(errno));
    return (NULL);
  }
  
  printer = parent_job->printer;
  _papplRWLockWrite(printer);
  ...
  if (NULL != parent_job->filename && 0 < strnlen(parent_job->filename, sizeof(parent_job->filename)))
  {
    ...
    {
      if ((newfd = papplJobOpenFile(new_job, new_job_file_name, sizeof(new_job_file_name), printer->system->directory, NULL, "w")) >= 0)
      {
