...
{
  // The ith entry in this array aligns with the ith in source and type below
  "na_letter_8.5x11in",
  "na_index-4x6_4x6in",
  "env_10_4.125x9.5in",
...
//-----------------------------------------------------------------------------------------------------
// 'epx_pappl_mime_filter_cb()' - MIME Filter callback for EPX.
bool                                              // O - `true` on success, `false` on failure
epx_pappl_mime_filter_cb(
                         ...
                         void           *data)    // I - Callback data
{
  ...
  (void) data; // Ignore this parameter
  
  papplLogJob(job, PAPPL_LOGLEVEL_INFO, "EPX Driver: Filtering Job '%s'(%s) for device %s",
              papplJobGetName(job),
              papplJobGetFilename(job),
              papplDeviceGetID(device, deviceID, sizeof(deviceID)));
                    ...
                    void                   *data)           // I - Callback data
{
  ...
  if (!data || strcmp((const char *)data, "testepx"))
  {
    papplLog(system, PAPPL_LOGLEVEL_ERROR, "EPX Driver(%s): Driver callback called with bad data pointer.", driver_name);
    return (false);
  }
  
  if (0 == strcmp(driver_name, "epx-driver-pwg-raster") || 0 == strcmp(driver_name, "epx-driver-pdf"))
  {
    papplLog(system, PAPPL_LOGLEVEL_INFO, "EPX Driver(%s): Using driver name '%s'.", driver_name, driver_name);
  }
  else
  {
  ...
  driver_data->duplex             = PAPPL_DUPLEX_NONE | PAPPL_DUPLEX_NORMAL;
  // TODO Why does pappl_pr_driver_data_s define separate fields for duplex and sides_supported
  ...
  // Media types - use MSN names
  // Ordered so the ith of type aligns with the ith of source
  driver_data->num_type = 0;
  driver_data->type[driver_data->num_type++] = "stationery";
  driver_data->type[driver_data->num_type++] = "photographic-glossy";
  driver_data->type[driver_data->num_type++] = "special";
  driver_data->type[driver_data->num_type++] = "transparency";
  driver_data->type[driver_data->num_type++] = "bond";
  
  // Input trays - use MSN names
  driver_data->num_source = 0;
  driver_data->source[driver_data->num_source++]  = "tray-1";
  driver_data->source[driver_data->num_source++]  = "photo";
  driver_data->source[driver_data->num_source++]  = "envelope";
  ...
  // NOTE: sources, types and media MUST be defined BEFORE this loop is run
  // The ith of source, type and pwg_common_media must be congruent with one another
  for (i = 0; i < driver_data->num_source; i ++)
  {
    pwg_media_t *pwg;                   /* Media size information */
    
    // The ith of source, type and pwg_common_media must be congruent with one another
    papplCopyString(driver_data->media_ready[i].size_name, pwg_common_media[i], sizeof(driver_data->media_ready[i].size_name));
    ...
    if ((pwg = pwgMediaForPWG(driver_data->media_ready[i].size_name)) != NULL)
    {
      ...
      strncpy(driver_data->media_ready[i].type, driver_data->type[i],  sizeof(driver_data->media_ready[i].type) - 1);
    }
  ...
  {
    // Office printer with finishings so multiple output bins...
    driver_data->bin[driver_data->num_bin++] = "auto";
    driver_data->bin[driver_data->num_bin++] = "right";
    driver_data->bin[driver_data->num_bin++] = "tray-1";
    driver_data->bin[driver_data->num_bin++] = "tray-2";
    driver_data->bin[driver_data->num_bin++] = "tray-3";
    driver_data->bin[driver_data->num_bin++] = "mailbox-1";
    driver_data->bin[driver_data->num_bin++] = "mailbox-2";
    driver_data->bin[driver_data->num_bin++] = "mailbox-3";
    driver_data->bin[driver_data->num_bin++] = "stacker-1";
    driver_data->bin[driver_data->num_bin++] = "stacker-2";
    driver_data->bin[driver_data->num_bin++] = "stacker-3";
  }
  else
  {
    // Simple printer gets one output bin...
    driver_data->bin[driver_data->num_bin++] = "center";
  }
  
  
  
  //-----------------------------------------------
  // Settings unique to each driver name
  if (0 == strcmp(driver_name, "epx-driver-pdf"))
  {
    ...
    if (NULL == epx_get_make_and_model_string(makeAndModelString, sizeof(makeAndModelString)))
      snprintf(makeAndModelString, sizeof(makeAndModelString), "UNKNOWN DON'T KNOW WHAT PDF Printer");
    ...
    // TODO If I don't include raster_types for PDF, validate_driver() will return false. Why is this necessary?
  ...
  else if (0 == strcmp(driver_name, "epx-driver-pwg-raster"))
  {
    // Printer attributes and information
    char makeAndModelString[256];
    if (NULL == epx_get_make_and_model_string(makeAndModelString, sizeof(makeAndModelString)))
      snprintf(makeAndModelString, sizeof(makeAndModelString), "UNKNOWN DON'T KNOW WHAT PWG Raster Printer");
    strncpy(driver_data->make_and_model, makeAndModelString, sizeof(driver_data->make_and_model) - 1);
          ...
          pappl_device_t     *device)   // I - Print device (unused)
{
  ...
  // TODO This needs to count pages to provide an accurate number
...
{
  pwg_job_data_t  *pwg = (pwg_job_data_t *)papplJobGetData(job);  // PWG driver data
  pappl_printer_t *printer = papplJobGetPrinter(job);             // Printer
  ...
  if (papplPrinterGetSupplies(printer, 5, supplies) == 5)
  {
    ...
    pappl_preason_t reasons = PAPPL_PREASON_NONE;   // "printer-state-reasons" values
    
    papplLogJob(job, PAPPL_LOGLEVEL_DEBUG, "EPX Driver: Calculating ink usage (%u,%u,%u,%u)",
                (unsigned)pwg->colorants[0],
                (unsigned)pwg->colorants[1],
                (unsigned)pwg->colorants[2],
                (unsigned)pwg->colorants[3]);
...
epx_rstartjob(
              ...
              pappl_device_t     *device)   // I - Print device (unused)
{
  pwg_job_data_t *pwg = (pwg_job_data_t *)calloc(1, sizeof(pwg_job_data_t)); // PWG driver data
  
  (void)options;
  
  papplLogJob(job, PAPPL_LOGLEVEL_INFO, "EPX Driver: Starting Job '%s' (%d): %s",
              papplJobGetName(job),
              papplJobGetID(job),
              papplJobGetFilename(job));
  papplLogJob(job, PAPPL_LOGLEVEL_INFO, "EPX Driver: Job '%s' (%d) - Options: %d pages; %d copies",
              papplJobGetName(job),
              papplJobGetID(job),
              options->num_pages,
              options->copies);
               ...
               const unsigned char *line)     // I - Line
{
  ...
  papplLogJob(job, PAPPL_LOGLEVEL_DEBUG, "Writing line for Job '%s': line number %d", papplJobGetName(job), y);
